...
| fcradius    | number                                         | The radius for the bounding Sphere used for Frustum Culling.  Default is 0, which means no frustum culling occurs.  When specified with a positive value, frustum culling is enabled on the mesh using a bounding sphere with this radius, and center fccenter (next property).  If set zero or negative, there is no frustum culling, and the whole mesh is rendered the whole time, even when off camera.  Note that frustum culling is all-or-nothing, applied to the whole mesh.  Frustum culling of individual members is not possible. | 0        |
| fccenter    | vec3                                           | The center for the bounding Sphere used for Frustum Culling.  This is only meaningful if fcradius is specified with a value greater than 0.  The format is 3 co-ordinates, separated with spaces (like a position), representing the X, Y and Z co-ordinates.  These co-ordinates are interpreted in the local position space of the entity the instanced-mesh is applied on (which may be different from world space). | none     |
...
4. If you wish to use Frustum Culling, it can only be applied to an entire Instanced Mesh.  Therefore if you have clusters of the same object, in multiple different areas, it may make sense to use different Instanced Meshes, so that one cluster can be frustum culled, without affecting the other.
...
- No support for [Skinned Meshes](https://threejs.org/docs/?q=skinnedMesh#api/en/objects/SkinnedMesh).  As of August 2023, there is no support in Three.js for instancing of Skinned Meshes, so supporting this would not be straightforward.  More details in [this issue](https://github.com/diarmidmackenzie/instanced-mesh/issues/30).
