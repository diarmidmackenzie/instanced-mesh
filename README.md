...
4. Instanced Meshes generated by this component cannot use Frustrum Culling.  This is where rendering can be skipped for objects completely outside the view of the camera don't get rendered at all.   The reason this isn't supported is that Frustrum Culling would have to be assessed for the entire mesh, and it gets quite complicated to reliably figure out whether any member of the Instanced Mesh is in view.   In many cases, Instanced Meshes are so much cheaper to render that this limitation is more than compensated for by the other gains made.  But there may be cases why that's not the case, and it's better to leave entities out of the Instanced Mesh, so they can be Frustrum Culled when not required.
...
With these decisions, I've been striking a balance between performance and complexity.  I've achieved the performance levels that I wanted to get to.  I'm pretty confident I could improve performance further, but if I'm already achieving 90 fps, there's not much benefit to that, and the further performance gains would come with increased complexity and risk.

If I need some more performance gains in the future, then I have some good options available that I could choose to exploit, but I don't see the need just yet.
